...
const packXYZ = (args) => {
    const srgb_linear = (0, srgb_1.xyz2rgbLinear)([args[0], args[1], args[2]]);
...
const extractHslComponents = (context, args) => {
    const tokens = args.filter(parser_1.nonFunctionArgSeparator), [hue, saturation, lightness, alpha] = tokens, h = (hue.type === 17 /* TokenType.NUMBER_TOKEN */ ? (0, angle_1.deg)(hue.number) : angle_1.angle.parse(context, hue)) / (Math.PI * 2), s = (0, length_percentage_1.isLengthPercentage)(saturation) ? saturation.number / 100 : 0, l = (0, length_percentage_1.isLengthPercentage)(lightness) ? lightness.number / 100 : 0, a = typeof alpha !== 'undefined' && (0, length_percentage_1.isLengthPercentage)(alpha) ? (0, length_percentage_1.getAbsoluteValue)(alpha, 1) : 1;
    return [h, s, l, a];
};
...
const packHSL = (context, args) => {
    ...
    const [h, s, l, a] = (0, exports.extractHslComponents)(context, args), rgb = hsl2rgb([h, s, l]);
    return (0, exports.pack)(rgb[0] * 255, rgb[1] * 255, rgb[2] * 255, s === 0 ? 1 : a);
};
exports.packHSL = packHSL;
const extractLchComponents = (args) => {
    const tokens = args.filter(parser_1.nonFunctionArgSeparator), l = (0, length_percentage_1.isLengthPercentage)(tokens[0]) ? tokens[0].number : 0, c = (0, length_percentage_1.isLengthPercentage)(tokens[1]) ? tokens[1].number : 0, h = (0, parser_1.isNumberToken)(tokens[2]) || (0, parser_1.isDimensionToken)(tokens[2]) ? tokens[2].number : 0, a = typeof tokens[4] !== 'undefined' && (0, length_percentage_1.isLengthPercentage)(tokens[4]) ? (0, length_percentage_1.getAbsoluteValue)(tokens[4], 1) : 1;
    return [l, c, h, a];
};
exports.extractLchComponents = extractLchComponents;
const extractLabComponents = (args) => {
    const tokens = args.filter(parser_1.nonFunctionArgSeparator), l = tokens[0].type === 16 /* TokenType.PERCENTAGE_TOKEN */
        ? tokens[0].number / 100
...
const hsl2rgb = ([h, s, l]) => {
    ...
    const t2 = l <= 0.5 ? l * (s + 1) : l + s - l * s, t1 = l * 2 - t2, r = (0, exports.hue2rgb)(t1, t2, h + 1 / 3), g = (0, exports.hue2rgb)(t1, t2, h), b = (0, exports.hue2rgb)(t1, t2, h - 1 / 3);
    return [r, g, b];
};
/**
 ...
 */
const oklab2xyz = (lab) => {
    const LMSg = (0, exports.multiplyMatrices)([
...
const rgbToXyz = (_context, args) => {
    ...
    if (tokens.length === 3) {
        const [r, g, b] = tokens.map(exports.getTokenColorValue), rgb_linear = (0, srgb_1.rgb2rgbLinear)([r / 255, g / 255, b / 255]), [x, y, z] = (0, srgb_1.rgbLinear2xyz)([rgb_linear[0], rgb_linear[1], rgb_linear[2]]);
        return [x, y, z, 1];
    }
    if (tokens.length === 4) {
        const [r, g, b, a] = tokens.map(exports.getTokenColorValue), rgb_linear = (0, srgb_1.rgb2rgbLinear)([r / 255, g / 255, b / 255]), [x, y, z] = (0, srgb_1.rgbLinear2xyz)([rgb_linear[0], rgb_linear[1], rgb_linear[2]]);
        return [x, y, z, a];
    }
    return [0, 0, 0, 1];
};
...
const hslToXyz = (context, args) => {
    const [h, s, l, a] = (0, exports.extractHslComponents)(context, args), rgb_linear = (0, srgb_1.rgb2rgbLinear)(hsl2rgb([h, s, l])), [x, y, z] = (0, srgb_1.rgbLinear2xyz)([rgb_linear[0], rgb_linear[1], rgb_linear[2]]);
    return [x, y, z, a];
};
...
const labToXyz = (_context, args) => {
    const [l, a, b, alpha] = (0, exports.extractLabComponents)(args), [x, y, z] = lab2xyz([l, a, b]);
    return [x, y, z, alpha];
};
...
const lchToXyz = (_context, args) => {
    const [l, c, h, alpha] = (0, exports.extractLchComponents)(args), [x, y, z] = lab2xyz(lch2lab([l, c, h]));
    return [x, y, z, alpha];
};
...
const oklchToXyz = (_context, args) => {
    const [l, c, h, alpha] = (0, exports.extractOkLchComponents)(args), [x, y, z] = oklab2xyz(lch2lab([l, c, h]));
    return [x, y, z, alpha];
};
...
const oklabToXyz = (_context, args) => {
    const [l, c, h, alpha] = (0, exports.extractLabComponents)(args), [x, y, z] = oklab2xyz([l, c, h]);
    return [x, y, z, alpha];
};
...
const xyz50FromXYZ = (args) => {
    const [x, y, z] = (0, exports.d65toD50)([args[0], args[2], args[3]]);
    return [x, y, z, args[3]];
};
...
const convertXyz50 = (args) => {
    const xyz = (0, exports.xyz50ToXYZ)([args[0], args[1], args[2]]);
